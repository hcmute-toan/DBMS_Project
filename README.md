...
3. **Cấu hình chuỗi kết nối** trong `App.config`
   ...
   - Mở file `App.config` và cập nhật chuỗi kết nối tới SQL Server của bạn:
     ...
     ```xml
     <connectionStrings>
         <add name="InventoryDbConnection" connectionString="Server=localhost;Database=InventoryDb;Trusted_Connection=True;" providerName="System.Data.SqlClient"/>
     </connectionStrings>
     ```
...
4. **Tạo cơ sở dữ liệu**
   ...
   - Chạy script SQL (`database.sql`) để tạo cơ sở dữ liệu và dữ liệu mẫu.
...
- Tài khoản đăng nhập mẫu:
  - Username: `admin1` / Password: `admin123` (vai trò: admin)
  - Username: `user1` / Password: `user123` (vai trò: user)
